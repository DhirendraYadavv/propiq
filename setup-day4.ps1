# PropIQ Day 4 - React Frontend Setup
Write-Host "Setting up Day 4 - React Frontend..." -ForegroundColor Cyan

$base = "C:\Users\User\propiq"
Set-Location $base
...
Write-Host "Installing dependencies done" -ForegroundColor Green
...
.bot-fab {
  ...
  box-shadow: 0 4px 24px #7c6af740;
  z-index: 100;
...
export default function PropBot({ onClose }) {
  ...
  const send = async () => {
    ...
    try {
      ...
      setMessages((m) => [...m, { role: "bot", text: "Sorry, PropBot is unavailable. Make sure it is running on port 8000." }]);
    } finally {
      setLoading(false);
    }
  };
...
.bot-close {
  background: none; border: none; color: var(--text2); padding: 4px;
  border-radius: 6px;
}
    ...
    Promise.all([propertyAPI.getAll(), tenantAPI.getAll(), leaseAPI.getAll()])
      .then(([p, t, l]) => {
        ...
        setStats({
          ...
          tenants: tens.length,
          leases: leas.length,
          occupied: props.filter((x) => x.isOccupied).length,
        });
    ...
    <div className="page">
      ...
      <div className="card" style={{ marginTop: 24 }}>
        ...
        {loading ? <div style={{ color: "var(--text2)" }}>Loading...</div> : leases.length === 0 ? (
          <div style={{ color: "var(--text2)", fontSize: 14 }}>No leases yet. Create one from Properties.</div>
        ) : (
          <table className="data-table">
            <thead>
              <tr><th>Property</th><th>Tenant</th><th>Monthly Rent</th><th>End Date</th><th>Status</th></tr>
            </thead>
            <tbody>
              {leases.map((l) => (
                <tr key={l.id}>
    ...
    <div className="page">
      ...
      {loading ? <div style={{ color: "var(--text2)" }}>Loading...</div> : (
        <div className="prop-grid">
          {properties.map((p) => (
            <div className="prop-card card" key={p.id}>
              <div className="prop-card-header">
                ...
                <span className={`badge ${p.isOccupied ? "badge-success" : "badge-info"}`}>
          ...
          {properties.length === 0 && <div style={{ color: "var(--text2)", fontSize: 14 }}>No properties yet. Add one!</div>}
        </div>
      )}
...
export default function Tenants() {
  ...
  return (
    <div className="page">
      <div className="page-header" style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start" }}>
        <div><h1>Tenants</h1><p className="page-sub">Manage your tenants — Aadhaar is always masked</p></div>
